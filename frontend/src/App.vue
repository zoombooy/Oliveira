  ...
  <div v-else class="app-shell" :class="{ 'is-collapsed': collapsed }">
    <aside class="sidebar">
      <div class="sidebar-top">
        ...
        <button class="icon-button sidebar-toggle" aria-label="折叠侧栏" @click="collapsed = !collapsed"><PanelLeft v-if="!collapsed" :size="17" /><PanelLeftOpen v-else :size="17" /></button>
      </div>
      ...
      <nav class="primary-nav" aria-label="主导航">
        ...
        <button :title="collapsed ? '对话' : undefined" :class="{ active: panel === 'chat' }" @click="panel = 'chat'"><span class="nav-icon"><MessageCircle :size="17" /></span><span v-if="!collapsed">对话</span><b v-if="!collapsed && conversations.length">{{ conversations.length }}</b></button>
        <button :title="collapsed ? '知识库' : undefined" :class="{ active: panel === 'documents' }" @click="panel = 'documents'"><span class="nav-icon"><LibraryBig :size="17" /></span><span v-if="!collapsed">知识库</span><b v-if="!collapsed">{{ documents.length }}</b></button>
        ...
        <button :title="collapsed ? '事实审核' : undefined" :class="{ active: panel === 'facts' }" @click="panel = 'facts'"><span class="nav-icon"><ClipboardList :size="17" /></span><span v-if="!collapsed">事实审核</span><b v-if="!collapsed && reviewItems.length" class="warning-count">{{ reviewItems.length }}</b></button>
        <button :title="collapsed ? '时间线' : undefined" :class="{ active: panel === 'timeline' }" @click="panel = 'timeline'"><span class="nav-icon"><GitBranch :size="17" /></span><span v-if="!collapsed">时间线</span></button>
        <button :title="collapsed ? '运行记录' : undefined" :class="{ active: panel === 'runs' }" @click="panel = 'runs'"><span class="nav-icon"><BarChart3 :size="17" /></span><span v-if="!collapsed">运行记录</span></button>
      </nav>
      ...
      <div v-if="!collapsed" class="conversation-list"><button v-for="conversation in conversations.slice(0, 6)" :key="conversation.id" :class="{ active: conversation.id === conversationId }" @click="selectConversation(conversation.id)"><MessageCircle :size="14" /><span>{{ conversation.title }}</span></button><div v-if="!conversations.length" class="empty-side">还没有对话</div></div>
    ...
    <main class="main-stage">
      ...
      <section v-else-if="panel === 'facts'" class="content"><div class="section-heading"><div><p class="eyebrow">待审核</p><h1>事实审核</h1><p>自动抽取的事实默认保持 pending，确认后才会进入回答上下文。</p></div><el-button plain @click="loadFacts">刷新</el-button></div><div class="review-layout"><div class="data-card review-card"><div v-for="item in reviewItems" :key="item.id" class="review-row"><div class="review-mark">待</div><div class="row-main"><strong>{{ facts.find(fact => fact.id === item.ref_id)?.subject_text }} · {{ facts.find(fact => fact.id === item.ref_id)?.predicate }}</strong><span>{{ facts.find(fact => fact.id === item.ref_id)?.object_text }}</span><small>{{ item.reason }}</small></div><button class="approve" @click="review(item, 'approve')">通过</button><button class="reject" @click="review(item, 'reject')">拒绝</button></div><div v-if="!reviewItems.length" class="empty-state compact"><ClipboardList :size="25" /><p>审核队列为空。</p></div></div><aside class="side-note"><h3>审核原则</h3><p>每条事实都必须能回到至少一个文档块。Oliveira 不会用新事实覆盖旧事实，冲突会单独保留。</p><span>当前事实 {{ facts.length }} 条</span></aside></div></section>
